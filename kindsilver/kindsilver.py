...
#discord bot tokken
token = 'NjkxNDU1OTc3MjcwMTQ5MTcx.XngOjw.Ow4c1by1uDQ11EHpXHxLi7uypaE'
...
async def on_message(message): # on_message() event : when the bot has recieved a message
    ...
    try:
        ...
        if level_up(temp_lv, int(dataRead(message.author.id))):
            ...
            await message.channel.send(":confetti_ball: Congratulations! **"+str(message.author)+"** has just arrived at **[Level "+level_show(int(dataRead(message.author.id)))+"]** !!!")
...
#남 정보 보기
    ...
    if message.content.startswith("/lv"):
        ...
        #최종 결과 임베드 타입으로 출력
        embed = discord.Embed(title="**"+str(user)+"**", description="", color=0xffff00)
        embed.add_field(name="레벨 (Level)", value=level_show(int(dataRead(auser))), inline=False)
        embed.add_field(name="경험치 (Xp)", value="{}/{} ({}%)\n*{}*  `{}` *{}*".format(temp_a, temp_b, round((100*temp_a/temp_b),2), temp, process_bar(temp_a/temp_b),temp+1), inline=False)
        embed.add_field(name="총 경험치 (Total Xp)", value=dataRead(auser), inline=False)
    ...
    if message.content.startswith("/목록"):
        ...
        for user_id in rankList():
            ...
            if int(dataRead(user_id)) < max_xp:
                await message.channel.send("**{}** | `{} / 800` | `Total : {}`".format(user, level(int(dataRead(user_id))), dataRead(user_id)))
            else:
